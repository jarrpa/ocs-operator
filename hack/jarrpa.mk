...
aws-ceph-ports: ##
	-export GROUP_ID="$$(aws ec2 describe-security-groups --output json --filters Name=tag:Name,Values=*-mr92t-worker-sg --query 'SecurityGroups[*].{ID:GroupId,Tags:Tags[?Key==`Name`].Value}' | jq -r '.[] | .ID')"; \
	echo "Group ID: $$GROUP_ID"; \
	aws ec2 authorize-security-group-ingress --group-id $${GROUP_ID} --protocol tcp --cidr 10.0.0.0/16 --port 31659 || true ; \   # Provider API Server
	aws ec2 authorize-security-group-ingress --group-id $${GROUP_ID} --protocol tcp --cidr 10.0.0.0/16 --port 9283  || true; \    # Ceph MGR
	aws ec2 authorize-security-group-ingress --group-id $${GROUP_ID} --protocol tcp --cidr 10.0.0.0/16 --port 6789 || true;  \    # Ceph MON v1
	aws ec2 authorize-security-group-ingress --group-id $${GROUP_ID} --protocol tcp --cidr 10.0.0.0/16 --port 3300 || true;  \    # Ceph MON v2
	aws ec2 authorize-security-group-ingress --group-id $${GROUP_ID} --protocol tcp --cidr 10.0.0.0/16 --port 6800-7300 || true;  # Ceph OSD
...
uninstall-provider: destroy-provider ## Uninstall OCS from provider
	...
	${OC_PROVIDER} delete -f upgrade-testing/namespace-setup-provider.yaml
