...
function gen_ocs_csv() {
	...
	popd
	$KUSTOMIZE build config/manifests | $OPERATOR_SDK generate bundle -q --overwrite=false --version "$CSV_VERSION" --channels "${OCS_SUBSCRIPTION_CHANNEL}" --default-channel "${OCS_SUBSCRIPTION_CHANNEL}"
